...
def to_csv_file(filename, headers, rows, delimiter = ',', new_line ='\n'):
    with open(filename, 'w', newline = new_line) as file:
        file.write((delimiter.join([element for element in headers])) + new_line)
        for row in rows:
            file.writelines((delimiter.join([r for r in row]) + new_line))
        return file
